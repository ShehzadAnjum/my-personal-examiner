...
- Q: Subject Configuration Schema - How should subject-specific configuration (marking criteria, extraction patterns) be stored for the generic/reusable system? → A: Option C - JSONB configuration in subjects table + resource files (marking templates, extraction patterns as separate files, metadata in DB). Provides balance of queryability (subject-level metadata in DB) and flexibility (complex templates as resource files). Economics configuration serves as template for other subjects.

- Q: Resource Ingestion Pipeline - How should the system auto-bootstrap when injecting a new subject? What resources are provided and how are they processed? → A: Option B - Hybrid: Manual upload of seed PDFs (10-20 past papers) + AI-powered resource analysis to generate subject configuration automatically. Admin provides sample resources (past papers), system uses AI to analyze patterns and auto-generate extraction_patterns, marking_config, paper_templates. For Economics: upload 10 papers → detect question patterns, marking structures → generate config. Same process for Math, English, etc.

- Q: Economics-Specific Asset Archival - Where and how should Economics 9708 configuration be preserved as the reference template for other subjects? → A: Option A - Store Economics config in both: (1) Live `subjects` table (operational), (2) `backend/resources/subjects/9708/` directory with README.md explaining it's the reference template for other subjects. Dual storage: DB config allows Economics to work immediately, archive in resources directory serves as reference for Math, English, etc. README documents patterns (e.g., "Economics uses level descriptors L1-L4 - adapt for subject-specific marking").

- Q: Multi-Subject Marking Engine Architecture - How should marking logic be structured for the generic system to mark across different subjects (Economics essays vs Math calculations vs English analysis)? → A: Combo of B+C (intelligently, efficiently, practically) - Marking engine framework with AI-powered subject-specific plugins. Base MarkingEngine provides common logic (score calculation, feedback formatting, caching). Subject plugins (Economics9708MarkingPlugin) provide: (1) AI prompts with subject-specific rubrics, (2) Fast rule-based checks for objective criteria (format, calculations, required elements), (3) AI marking for subjective evaluation (essays, analysis, evaluation). Efficient: Cache AI responses, structured prompts minimize tokens. Practical: Fallback to rules when AI unavailable. Economics plugin serves as reference implementation for Phase III.

- Q: Phase II Scope with Generic Architecture - Should Phase II include full AI-powered subject bootstrap functionality, or focus on infrastructure + Economics MVP? → A: Option B - Phase II builds infrastructure + Economics implementation, defer full bootstrap to Phase V. Phase II delivers: (1) Generic extraction framework with plugin architecture, (2) Economics 9708 fully working (archived config as reference template), (3) Database schema with JSONB config fields, (4) Subject plugin architecture proven. Phase V adds: `/api/subjects/bootstrap` endpoint with AI analysis for new subjects. Rationale: Prove generic architecture with Economics first, add automation later. Practical MVP approach: working system sooner.
...
**Subject Configuration** (UPDATED per clarifications 2025-12-18):
- **FR-016**: System MUST store subject configuration in JSONB fields (marking_config, extraction_patterns, paper_templates) within `subjects` table
- **FR-017**: System MUST load complex templates from resource files at `backend/resources/subjects/{subject_code}/` (marking_prompts.md, extraction_rules.yaml)
- **FR-018**: System MUST use Economics 9708 configuration as reference template when adding new subjects
- **FR-019**: System MUST implement generic extraction framework that uses subject config (not hard-coded per-subject logic)
- **FR-020**: Economics 9708 configuration MUST be manually created and archived in `backend/resources/subjects/9708/` as reference
- **FR-021**: Database migration MUST add marking_config, extraction_patterns, paper_templates JSONB columns to `subjects` table

**Syllabus Management**:
- **FR-022**: System MUST store Economics 9708 syllabus hierarchy in `syllabus_points` table (seeded from Cambridge website)
- **FR-023**: System MUST support syllabus querying via `/api/subjects/{id}/syllabus` returning hierarchical JSON structure
- **FR-024**: System MUST track syllabus version per subject via `subjects.syllabus_year` field

**Mark Scheme Matching**:
- **FR-025**: System MUST automatically match mark scheme PDFs to question paper PDFs via (subject_code, year, session, paper_number) tuple
- **FR-026**: System MUST support retroactive mark scheme matching (mark scheme uploaded before questions)

**API Endpoints** (RESTful):
- **FR-027**: `POST /api/questions/upload` - Upload single PDF (question paper or mark scheme), returns 201 with extracted questions count
- **FR-028**: `GET /api/questions` - Search questions with filters (subject_id, syllabus_points, difficulty, year_min, year_max, marks_min, marks_max), returns paginated results
- **FR-029**: `GET /api/questions/{id}` - Retrieve single question with full metadata and marking scheme
- **FR-030**: `PATCH /api/questions/{id}/tags` - Update syllabus_point_ids for question
- **FR-031**: `POST /api/exams/generate` - Generate exam with criteria, returns 201 with exam object including selected question IDs
- **FR-032**: `GET /api/subjects/{id}/syllabus` - Retrieve hierarchical syllabus structure

**Error Handling**:
- **FR-034**: System MUST return 400 Bad Request for invalid PDF formats or non-Cambridge filenames with specific error messages
- **FR-035**: System MUST return 409 Conflict when uploading duplicate questions (same source_paper + question_number)
- **FR-036**: System MUST return 422 Unprocessable Entity when exam generation criteria cannot be met due to insufficient questions
...
**Subject**: Represents an A-Level subject with its configuration (UPDATED per clarification 2025-12-18)
- Attributes: id (UUID), code (STRING, e.g., "9708"), name (STRING, e.g., "Economics"), level (STRING: "AS"/"A"), exam_board (STRING), syllabus_year (STRING), **marking_config (JSONB)**, **extraction_patterns (JSONB)**, **paper_templates (JSONB)**, created_at
- **marking_config**: Subject-specific marking criteria metadata (e.g., `{rubric_type: "level_descriptors", max_levels: 4, essay_structure: true}` for Economics)
- **extraction_patterns**: Question extraction rules metadata (e.g., `{question_delimiter: "Question \\d+", marks_pattern: "\\[\\d+ marks\\]", has_subparts: true}`)
- **paper_templates**: Paper structure definitions (e.g., `{papers: [{paper_number: 3, questions_count: 3, marks_per_question: 25, duration: 90}]}`)
- **Resource Files**: Complex templates stored in `backend/resources/subjects/{subject_code}/` (marking_prompts.md, extraction_rules.yaml, etc.)
- Relationships: Has many Questions, has many SyllabusPoints, has many Exams
- Economics 9708 configuration serves as template for other Cambridge A-Level subjects
...
- ❌ AI-powered marking of student answers (Phase III - but architecture defined in clarifications for Phase II planning)
- ❌ AI-powered subject bootstrapping (`/api/subjects/bootstrap` endpoint deferred to Phase V per clarification 2025-12-18)
- ❌ Automated syllabus crawling from Cambridge website (manual seed data OK)
- ❌ Support for subjects other than Economics 9708 (MVP subject only - but generic architecture established in Phase II)
- ❌ OCR or image analysis for diagrams (placeholder text only)
- ❌ Automated syllabus tagging via AI (manual tagging only in Phase II, auto-tagging in Phase V)
...
**Phase III Preview** (Architecture defined per clarification 2025-12-18):
- ✅ Marking engine framework architecture decided: Plugin-based with AI integration
- ✅ Economics9708MarkingPlugin will be reference implementation
- ✅ Rule-based checks for objective criteria + AI for subjective evaluation

**Phase V Preview** (Deferred per clarification 2025-12-18):
- ⏳ `/api/subjects/bootstrap` endpoint - AI-powered subject configuration generator
- ⏳ Upload 10-20 seed PDFs for new subject → AI analyzes patterns → auto-generates config
- ⏳ Admin reviews/approves generated configuration before subject goes live
- ⏳ Economics 9708 archived config serves as reference for AI to learn patterns
...
**Code Deliverables**:
- [ ] `backend/src/question_extractors/cambridge_parser.py` - Filename parsing utility (generic)
- [ ] `backend/src/question_extractors/generic_extractor.py` - Generic PDF question extraction (uses subject config from JSONB)
- [ ] `backend/src/question_extractors/mark_scheme_extractor.py` - Mark scheme parsing (generic)
...
- [ ] `backend/alembic/versions/002_add_questions_and_subject_config.py` - Database migration (add questions, exams, pdf_extraction_logs tables + add marking_config, extraction_patterns, paper_templates JSONB columns to subjects)

**Economics 9708 Reference Archive** (per clarification 2025-12-18):
- [ ] `backend/resources/subjects/9708/README.md` - Economics as reference template documentation
- [ ] `backend/resources/subjects/9708/marking_config.json` - Economics level descriptors (L1-L4), essay structure rules
- [ ] `backend/resources/subjects/9708/extraction_patterns.yaml` - Question delimiters, marks notation, subpart patterns
- [ ] `backend/resources/subjects/9708/paper_templates.json` - Paper 1/2/3 structures, question counts, marks distribution
- [ ] `backend/resources/subjects/9708/sample_papers/` - 10+ Economics past papers used for testing
